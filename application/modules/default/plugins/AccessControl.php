...
protected function _getAcl()
{
	...
	if(!isset($role))
	   ...
	   if($role == 2 )
		 ...
		 $acl->addResource(new Zend_Acl_Resource('default:appraisalmanager'));
                            $acl->allow('employee', 'default:appraisalmanager', array('submitmanager','deletemanagergroup','savemanagergroup','index','viewgroup','createnewgroup','showgroups','showviewgroups','view','Manager Appraisal'));

		 $acl->addResource(new Zend_Acl_Resource('default:appraisalself'));
                            $acl->allow('employee', 'default:appraisalself', array('index','save','view','Self Appraisal'));

		 $acl->addResource(new Zend_Acl_Resource('default:businessunits'));
                            $acl->allow('employee', 'default:businessunits', array('index','getdeptnames','view','Business Units'));

		 $acl->addResource(new Zend_Acl_Resource('default:clients'));
                            $acl->allow('employee', 'default:clients', array('index','addpopup','view','Clients'));
		 ...
		 $acl->addResource(new Zend_Acl_Resource('default:leaverequest'));
                            $acl->allow('employee', 'default:leaverequest', array('index','saveleaverequestdetails','gethalfdaydetails','editpopup','updateleavedetails','Leave Request'));

		 $acl->addResource(new Zend_Acl_Resource('default:manageremployeevacations'));
                            $acl->allow('employee', 'default:manageremployeevacations', array('index','view','Employee Leave'));

		 $acl->addResource(new Zend_Acl_Resource('default:mydetails'));
                            $acl->allow('employee', 'default:mydetails', array('index','personaldetailsview','personal','communicationdetailsview','communication','skills','education','experience','leaves','holidays','salarydetailsview','certification','creditcarddetailsview','creditcard','visadetailsview','visa','medicalclaims','disabilitydetailsview','disability','dependency','workeligibilitydetailsview','workeligibility','additionaldetailsedit','jobhistory','documents','assetdetailsview','view','My Details'));
		 ...
		 $acl->addResource(new Zend_Acl_Resource('default:projects'));
                            $acl->allow('employee', 'default:projects', array('index','viewpopup','editpopup','view','Projects'));

		 $acl->addResource(new Zend_Acl_Resource('default:scheduleinterviews'));
                            $acl->allow('employee', 'default:scheduleinterviews', array('candidatepopup','index','downloadresume','getcandidates','view','Interviews'));
		 ...
		 $acl->addResource(new Zend_Acl_Resource('expenses:advances'));
                            $acl->allow('employee', 'expenses:advances', array('index','getprojects','myadvances','viewmoreadvances','clearadvancesdata','addreturnpopup','view','Advances'));

		 $acl->addResource(new Zend_Acl_Resource('expenses:employeeadvances'));
                            $acl->allow('employee', 'expenses:employeeadvances', array('index','view','Employee Advances'));

		 $acl->addResource(new Zend_Acl_Resource('expenses:expenses'));
                            $acl->allow('employee', 'expenses:expenses', array('index','clone','addpopup','uploadsave','uploaddelete','displayreceipts','addtrippopup','submitexpense','addreceiptimage','expensestatus','listreportingmangers','viewmoremanagers','forwardexpenseto','downloadexpensepdf','bulkexpenses','getcategories','getprojects','getcurrency','uploadedfiles','view','Expenses'));

		 $acl->addResource(new Zend_Acl_Resource('expenses:myemployeeexpenses'));
                            $acl->allow('employee', 'expenses:myemployeeexpenses', array('index','view','My Employee Expenses'));

		 $acl->addResource(new Zend_Acl_Resource('expenses:receipts'));
                            $acl->allow('employee', 'expenses:receipts', array('index','downloadreceipt','downloadexpensereceipt','deletereceipt','uploadsave','displayreceipts','viewmorereceipts','listexpenses','addreceipttoexpense','viewmoreexpenses','cleardata','showreceiptspopup','listtrips','viewmoretrips','addexpensetotrip','view','Receipts'));

		 $acl->addResource(new Zend_Acl_Resource('expenses:trips'));
                            $acl->allow('employee', 'expenses:trips', array('index','addpopup','tripstatus','deleteexpense','downloadtrippdf','view','Trips'));

		 $acl->addResource(new Zend_Acl_Resource('exit:allexitproc'));
                            $acl->allow('employee', 'exit:allexitproc', array('index','editpopup','updateexitprocess','assignquestions','view','All Exit Procedures'));

		 $acl->addResource(new Zend_Acl_Resource('exit:exitproc'));
                            $acl->allow('employee', 'exit:exitproc', array('index','questions','savequestions','view','Initiate/Check Status'));
